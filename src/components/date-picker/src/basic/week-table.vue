...
const clearDate = (date) => {
    return m(date).startOf('W').toDate();
};
...
export default {
    ...
    methods: {
        ...
        getWeekOfCell(w) {
            const year = this.date.getFullYear();
            return m(year + '-' + (w + 1), 'YYYY-W').toDate();
        },
        markRange(minDate, maxDate) {
            ...
            for (let i = 0, k = rows.length; i < k; i++) {
                ...
                for (let j = 0, l = row.length; j < l; j++) {
                    ...
                    const time = m(this.date.getFullYear() + '-' + (index + 1), 'YYYY-W').toDate().getTime();
        ...
        handleMouseMove(event) {
            ...
            if (row !== this.lastRow || column !== this.lastColumn) {
                ...
                this.$emit('changerange', {
                    ...
                    rangeState: {
                        selecting: true,
                        endDate: this.getWeekOfCell(row * 10 + column)
                    }
                });
            }
        },
        ...
        rows() {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                for (let j = 0; j < 10; j++) {
                    ...
                    cell.text = index + 1;
                    cell.type = 'normal';
                    let time = m(year + '-' + cell.text, 'YYYY-W').toDate().getTime();
